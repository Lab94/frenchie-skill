...
> Upgrading from 0.1.x, 0.2.x, 0.3.0, 0.3.1, 0.3.2, 0.4.0, 0.4.1, 0.4.2, 0.4.3, or 0.4.4? Follow the matching section below, in order. Each section is self-contained — do the steps in order and you'll be back on the happy path.
...
Restart your agent after `install` finishes so it re-reads the pinned `@lab94/frenchie@0.4.5` spec. If you hand-pinned the version, update to `@lab94/frenchie@0.4.5` or re-run `install`.

---

## 0.4.3 → 0.4.4

0.4.4 fixes async image-generation result handling. Failed image jobs now surface as errors from `get_job_result` instead of looking like they are still processing, and generated-image retention starts when the result is first delivered.
...
Restart your agent after `install` finishes so it re-reads the pinned `@lab94/frenchie@0.4.4` spec. If you hand-pinned the version, update to `@lab94/frenchie@0.4.4` or re-run `install`.
...
0.4.2 is a metadata-only release that fixes the Official MCP Registry namespace (`io.github.Lab94/frenchie-skill`) so the skill listing publishes correctly. No tool semantics, response shapes, or installer behaviour change. Re-run `install` to pick up the new pinned version, but nothing else needs to change.
...
Restart your agent after `install` finishes so it re-reads the pinned `@lab94/frenchie@0.4.2` spec.
